...
function convertFractions(text) {
  // Pattern to match fractions in various formats:
  // $a/b$, $(-a/b)$, $(a/b)$, etc.
  const fractionPattern = /\$([^$]*?)([+-]?\d+\/\d+)([^$]*?)\$/g;
  
  return text.replace(fractionPattern, (match, before, fraction, after) => {
...
function main() {
  ...
  console.log('🔧 Starting fraction conversion process for specific files...');
