...
function convertAlgebraicFractions(text) {
  ...
  // Pattern 6: Fix cases where \\frac{1}{x}^2 should be \\frac{1}{x^2}
  result = result.replace(/\\frac\{1\}\{x\}\^(\d+)/g, (match, power) => {
    return `\\frac{1}{x^${power}}`;
  });
  
  result = result.replace(/\\frac\{1\}\{y\}\^(\d+)/g, (match, power) => {
    return `\\frac{1}{y^${power}}`;
  });
...
function main() {
  ...
  console.log('🔧 Starting final algebraic fraction conversion process for multiplying-polynomials.json...');
