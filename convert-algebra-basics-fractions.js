...
function convertFractions(text) {
  ...
  // Pattern 4: Variable fractions like $x/2$, $y/3$, etc.
  result = result.replace(/\$([^$]*?)([a-zA-Z])\/(\d+)([^$]*?)\$/g, (match, before, variable, denominator, after) => {
    return `$${before}\\frac{${variable}}{${denominator}}${after}$`;
  });
  
  // Pattern 5: Variable fractions with negative signs like $-x/2$, $-y/3$, etc.
  result = result.replace(/\$([^$]*?)(-[a-zA-Z])\/(\d+)([^$]*?)\$/g, (match, before, variable, denominator, after) => {
    return `$${before}\\frac{${variable}}{${denominator}}${after}$`;
  });
  
  // Pattern 6: Complex variable fractions like $a/b$, $x/y$, etc.
  result = result.replace(/\$([^$]*?)([a-zA-Z])\/([a-zA-Z])([^$]*?)\$/g, (match, before, numerator, denominator, after) => {
    return `$${before}\\frac{${numerator}}{${denominator}}${after}$`;
  });
  
  // Pattern 7: Fractions with variables and numbers like $2x/5$, $3y/4$, etc.
  result = result.replace(/\$([^$]*?)(\d+[a-zA-Z])\/(\d+)([^$]*?)\$/g, (match, before, numerator, denominator, after) => {
    return `$${before}\\frac{${numerator}}{${denominator}}${after}$`;
  });
  
  // Pattern 8: Fractions with parentheses like $1/(yz)$, $2/(xy)$, etc.
  result = result.replace(/\$([^$]*?)(\d+)\/(\([^)]+\))([^$]*?)\$/g, (match, before, numerator, denominator, after) => {
    return `$${before}\\frac{${numerator}}{${denominator}}${after}$`;
  });
...
function main() {
  ...
  const specificFile = path.join(skillsDirectory, 'algebra-basics.json');
  
  console.log('🔧 Starting fraction conversion process for algebra-basics.json...');
